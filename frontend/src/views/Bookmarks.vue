<template>
  <v-container>
    <v-row>
      <div class="text-h5">Bookmarks</div>
    </v-row>
    <v-row class="mt-6">
      <v-col cols="6">
        <v-combobox
    ...
    <template v-for="orgSecRef of filteredOrgSecondaryReferences" :key="orgSecRef._id">
      <br>
      <v-row clas="ml-4 mr-4" dense>
        <div class="text-h6">{{ getOrganization(orgSecRef.organizationId)?.name }}</div>
      </v-row>
      <v-row v-if="orgSecRef.bookmarks.length" dense>
        <template
        ...
        >
          ...
          >
            <v-card
              ...
              width="344"
            >
              <router-link :to="{ name: 'Share', params: { id: bookmark.collectionSummary._id } }" style="text-decoration: none;">
              ...
              <v-card-actions>
                <v-btn
                  ...
                  variant="text"
                  :to="{ name: 'Share', params: { id: bookmark.collectionSummary._id } }"
                >
                  Explore
                </v-btn>
                <v-spacer></v-spacer>
      ...
      <v-row v-if="!orgSecRef.bookmarks.filter(b => b.collectionName === bookmarkType.collectionName).length" class="ml-4 mr-4">
...
export default {
  name: "Bookmarks",
  components: { ManageBookmarkDialog },
  data() {
    return {
